...
from torchmetrics import ConfusionMatrix, PearsonCorrCoef
...
class PlOnehotWrapper(pl.LightningModule):
    ...
    def xarray_encode(self, x: xr.DataArray) -> xr.DataArray:
        ...
        transforming it to a torch.Tensor and calling the forward method.
    ...
    def log_conf_matrix(self, conf_matrix: torch.Tensor) -> None:
        ...
        cmat = sns.heatmap(conf_matrix, annot=True)
        ...
        plt.clf()
        fig, ax = plt.subplots(figsize=(11.7, 8.27))
        cmat = sns.heatmap(conf_matrix_norm, annot=True)
